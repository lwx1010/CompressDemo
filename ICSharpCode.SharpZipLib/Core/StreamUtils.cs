...
namespace ICSharpCode.SharpZipLib.Core
{
	...
	public sealed class StreamUtils
	{
        ...
        {
            Copy(source, destination, buffer, null);
        }

        static public void Copy(Stream source, Stream destination, byte[] buffer,YZL.Compress.GZip.GZipFile.CodeProgress progress)
		{
			if (source == null) {
            ...
            long allLength = source.Length;
			while (copying) {
				int bytesRead = source.Read(buffer, 0, buffer.Length);
				if (bytesRead > 0) {
					destination.Write(buffer, 0, bytesRead);
                    progressLength += bytesRead;

                    if(progress != null)
                        progress.SetProgressPercent(allLength, progressLength);
				} else {
					destination.Flush();
					copying = false;

                    if (progress != null)
                        progress.SetProgressPercent(allLength, allLength);
				}
			}
		}
